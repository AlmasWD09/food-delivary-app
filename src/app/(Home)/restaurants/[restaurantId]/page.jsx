...
export default function RestaurantD({ params }) {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/restaurents/${params.restaurantId}`)
      .then(res => res.json())
      .then(data => setRestaurant(data))
  }, [params])

  // const restaurant = restaurants.find(restaurent => restaurent._id === )
        ...
        <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {
            data?.length > 0 ? data?.map(review => <>
              ...
              <h1 className="text-xl font-semibold my-2 text-center">{review?.userName}</h1>
              <h1 className="text-sm text-center">{review?.review}</h1>
            </div>
              </>) : 
              <>
              <div className="p-10">
              <h4 className="text-s text-center">This restaurant have not any reviews</h4>
              {
                isLoading && <p><FaSpinner className="animate-spin" /></p>
              }
              </div>
              </>
          }
